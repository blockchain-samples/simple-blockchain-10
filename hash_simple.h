...
{   //compress two longs into one using bit operations 
    return (arg1 * arg2) ^ ((arg1 << 3) + (arg2 >> 2));
}

long ID_hash(string name)
{   //name will be 8 characters + Case 
...
unsigned long block_hash(unsigned long merkle_hash, unsigned long timestamp, \
                        unsigned long prev_hash, unsigned long number, bool valid)
{
